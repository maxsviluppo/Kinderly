...
const AttendanceSection: React.FC<AttendanceProps> = ({ selectedClassId = 'all' }) => {
  ...
  const handleEarlyExit = (studentId: string) => {
    const delegateName = prompt("Inserisci nome del delegato al ritiro:");
    ...
    setAttendanceRecords(prev => {
      ...
      if (existingIdx > -1) {
        updatedRecords[existingIdx] = {
          ...
          delegatedPickup: delegateName,
          notes: (updatedRecords[existingIdx].notes || '') + ` | Uscita Ant. ore ${currentTime}`
        };
      } else {
        // Assume present if exiting early but no record? Or create one.
        updatedRecords.push({
          id: `new-${Date.now()}`,
          studentId,
          date: today,
          ...
          departureTime: currentTime,
          delegatedPickup: delegateName,
          notes: `Uscita Ant. ore ${currentTime}`
        });
      }
      return updatedRecords;
    });
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredStudentsList.map(student => {
                  ...
                  const isLate = record?.notes?.includes('Ritardo');
                  const isEarlyExit = record?.isEarlyExit;
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {isEarlyExit && <span className="text-[9px] font-extrabold text-orange-600 bg-orange-100 px-1.5 py-0.5 rounded ml-1">USCITA ANTICIPATA</span>}
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex gap-2">
                          <button onClick={() => updateStatus(student.id, 'present')} className={`p-2 rounded-lg border transition-all ${record?.status === 'present' && !isEarlyExit ? 'bg-emerald-500 border-emerald-600 text-white' : 'bg-white text-slate-300'}`}><Check size={16} /></button>
                          <button onClick={() => handleQuickDelay(student.id)} className={`p-2 rounded-lg border transition-all ${isLate ? 'bg-amber-500 border-amber-600 text-white' : 'bg-white text-slate-300'}`} title="Ritardo"><AlertCircle size={16} /></button>
                          <button onClick={() => handleEarlyExit(student.id)} className={`p-2 rounded-lg border transition-all ${isEarlyExit ? 'bg-orange-500 border-orange-600 text-white' : 'bg-white text-slate-300'}`} title="Uscita Anticipata"><LogOut size={16} /></button>
                          <button onClick={() => updateStatus(student.id, 'absent')} className={`p-2 rounded-lg border transition-all ${record?.status === 'absent' ? 'bg-rose-500 border-rose-600 text-white' : 'bg-white text-slate-300'}`} title="Assenza"><X size={16} /></button>
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-600">{record?.arrivalTime || '--:--'}</span>
                          {isEarlyExit && <span className="text-[10px] font-bold text-orange-500">Uscita: {record?.departureTime}</span>}
                        </div>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-xs italic text-slate-400">{record?.notes || '-'}</span>
                          {isEarlyExit && record?.delegatedPickup && <span className="text-[9px] text-slate-500 font-medium">Delegato: {record.delegatedPickup}</span>}
                        </div>
                      </td>
                            ...
                            <td key={dateStr} className={`p-1 border-l border-slate-100 text-center ${isWeekend ? 'bg-slate-50/30' : ''}`}>
                              {isWeekend ? (
                                ...
                                <button
                                  ...
                                  className={`w-7 h-7 rounded-lg flex items-center justify-center mx-auto transition-all ${record?.status === 'present' ? 'bg-emerald-500 text-white' :
